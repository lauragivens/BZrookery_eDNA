...
ps.bold <- readRDS(paste0(dir_results,'/ps.bold.rds'))
...
```{r load_env}
load(paste0(dir_data,'/07_BOLDBlanks.RData'))
```
...
```{r}
ps.blanks <- ps.bold %>% subset_samples(.,Rookery=='Blank') %>% prune_taxa(taxa_sums(.)>0,.)
...
```{r}
ntaxa(prune_taxa(taxa_names(ps.bold)%in%blank.asvs,ps.bold)) / ntaxa(ps.bold) * 100
```
4.217% of taxa    

What percent of sequences do those ASVs comprise?  
```{r}
sum(taxa_sums(prune_taxa(taxa_names(ps.bold)%in%blank.asvs,ps.bold))) / sum(taxa_sums(ps.bold)) * 100
```
84.89% of the sequences  

And how many of those are in the blank samples themselves?  
```{r}
sum(taxa_sums(ps.blanks)) / sum(taxa_sums(ps.bold)) * 100
```
1.17% of ASVs are found in the blanks
...
```{r}
ps.bold %>%  #sample sums when subset to only species in the blanks
  subset_taxa(.,Species%in%(get_taxa_unique(ps.blanks,"Species"))) %>% 
  sample_sums() %>% summary()

(ps.bold %>% prune_taxa(!taxa_names(.)%in%taxa_names(ps.blanks),.)) %>%  #sample sums when subset to only species in the blanks
  subset_taxa(.,Species%in%(get_taxa_unique(ps.blanks,"Species"))) %>% 
  sample_sums() %>% summary()
```
...
```{r}
ps.decontam <- ps.bold %>% subset_samples(Site != "Practice") %>% prune_samples(sample_sums(.)>0,.)
...
```{r}
ps.decontam %>% prune_taxa(taxa_names(.)%in%contamnames.freq,.) %>% subset_taxa(!is.na(Phylum)) %>% tax_table() %>% as.matrix() %>% as.data.frame() %>% .[2:8]
```

What does that look like?  
```{r}
plot_frequency((ps.decontam  %>%  subset_samples(Quant_nguL>0)), 
               contamnames.freq[1:20], conc="Quant_nguL") + 
...
```{r}
ps.decontam %>% prune_taxa(taxa_names(.)%in%contamnames.prev,.)%>% subset_taxa(!is.na(Phylum)) %>% tax_table() %>% as.matrix() %>% as.data.frame() %>% .[2:8]
```
...
What percent of taxa do those ASVs comprise?  
```{r}
length(contamdf.names) / ntaxa(ps.bold) * 100
```
0.683% of taxa     

What percent of sequences do those ASVs comprise?  
```{r}
sum(taxa_sums(prune_taxa(taxa_names(ps.bold)%in%contamdf.names,ps.bold))) / sum(taxa_sums(ps.bold)) * 100
```
0.20% of the sequences  

```{r}
sample_sums(ps.bold %>% prune_taxa(taxa_names(.)%in%contamdf.names,.)) %>% summary()
```

Remove samples with less than 
```{r}
decontam.ps.bold <- ps.bold %>% 
...
```{r}
ps.blanks <- decontam.ps.bold %>% subset_samples(.,Rookery=='Blank') %>% prune_taxa(taxa_sums(.)>0,.)
```
...
PCR blanks:  
```{r}
ps.blanks %>% subset_samples(Site=="PCR") %>% prune_taxa(taxa_sums(.)>0,.) %>% otu_table() %>% .[1:10,]
```

Remove ASVs from extraction blanks from their respective extraction day  
```{r}
ps.blanks %>% subset_samples(Site=="Extraction")  %>% prune_taxa(taxa_sums(.)>0,.) %>% otu_table() %>% .[1:10,]
```

Remove ASVs from field and lab blanks from their respective sample days   
```{r}
ps.blanks %>% subset_samples(Site=="Lab" | Site=="Field")  %>% prune_taxa(taxa_sums(.)>0,.) %>% otu_table() %>% .[1:10,]
```
```{r}
...
#in neg
otu_table(decontam.ps.bold)[b[1],c]

#before
otu_table(decontam.ps.bold)[b[1],a] #samples from that sampling event
...
```
```{r}
otu_table(ps.blanks %>% subset_samples(Site=="PCR") %>% prune_taxa(taxa_sums(.)>0,.)) %>% .[1:10]
```
Test: 
```{r,eval=FALSE}
...
```{r}
#in neg
otu_table(prune_b.ps)[b,c] %>% rowSums() %>% .[1:10]

#before
otu_table(prune_b.ps)[b[1:10],a[1:5]] #samples from that sampling event

#after
otu_table(prune_c.ps)[b[1:10],a[1:5]]
```

### Final
```{r}
prune.decontam.ps.bold <- prune_c.ps %>% subset_samples(Rookery!="Blank")
```

# Overview   
Blanks  
with na taxa included
```{r}
ov.og.blana <- ps.blanks %>% plot_bar(x="SampleName_Short",fill="Family") + geom_bar(stat="identity",position="stack") + facet_grid(~Site,scales="free") + labs(title="ASV abundance of negative controls and blanks",x="Sample Name") + theme(legend.position="none")
```
with na taxa removed  
```{r}
ov.og.bla <- ps.blanks %>% subset_taxa(!is.na(Phylum)) %>% plot_bar(x="SampleName_Short",fill="Family") + geom_bar(stat="identity",position="stack") + facet_grid(~Site,scales="free") + labs(title="ASV abundance of negative controls and blanks",x="Sample Name") + theme(legend.position="none")
```

All samples
Before decontaminant removal  
```{r}
ov.decon.bef.na <- ps.bold %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
  plot_bar(x="SampleName_Short",fill="Family") + geom_bar(stat="identity",position="stack") + facet_grid(~Site,scales="free") + 
  labs(title="Abundance of ASVs found in negative controls in total dataset",x="Sample Name") + theme(legend.position="none")
```
```{r}
ov.decon.bef <- ps.bold %>% subset_taxa(!is.na(Phylum)) %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
  plot_bar(x="SampleName_Short",fill="Family") + geom_bar(stat="identity",position="stack") + facet_grid(~Site,scales="free") + 
  labs(title="Abundance of ASVs found in negative controls in total dataset",x="Sample Name") + theme(legend.position="none")
```
...
```{r}
ov.decon.na <- decontam.ps.bold  %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
  plot_bar(x="SampleName_Short",fill="Family") + 
  geom_bar(stat="identity",position="stack") + facet_grid(~Site,scales="free") + 
  labs(title="Abundance of ASVs found in negative controls in total dataset after decontaminant removal",x="Sample Name") + theme(legend.position="none")
```
```{r}
ov.decon <- decontam.ps.bold %>% subset_taxa(!is.na(Phylum))  %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
  plot_bar(x="SampleName_Short",fill="Family") + 
  geom_bar(stat="identity",position="stack") + facet_grid(~Site,scales="free") + 
  labs(title="Abundance of ASVs found in negative controls in total dataset after decontaminant removal",x="Sample Name") + theme(legend.position="none")
```
...
```{r}
ov.neg.na <- prune.decontam.ps.bold %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
  plot_bar(x="SampleName_Short",fill="Genus") + facet_grid(~Site,scales="free") + 
  labs(title="Abundance of ASVs found in negative controls in total dataset after subtracting number of ASVs in negative controls ",x="Sample Name")+ theme(legend.position="none")

ov.neg <- prune.decontam.ps.bold  %>% subset_taxa(!is.na(Phylum)) %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
  plot_bar(x="SampleName_Short",fill="Genus") + facet_grid(~Site,scales="free") + 
  labs(title="Abundance of ASVs found in negative controls in total dataset after subtracting number of ASVs in negative controls ",x="Sample Name")+ theme(legend.position="none")
```

```{r}
cowplot::plot_grid(ov.decon.bef.na,ov.decon.na,ov.neg.na)
```
```{r}
cowplot::plot_grid(ov.decon.bef,ov.decon,ov.neg)
```

# Session Info   
```{r}
saveRDS(decontam.ps.bold,paste0(dir_results,"/decontam.ps.bold.rds"))

saveRDS(prune_c.ps,paste0(dir_results,"/prune.decontam.ps.bold.keepblanks.rds"))
saveRDS(prune.decontam.ps.bold,paste0(dir_results,"/prune.decontam.ps.bold.rds"))
```

```{r}
save.image(paste0(dir_data,'/07_BOLDBlanks.RData'))
```
