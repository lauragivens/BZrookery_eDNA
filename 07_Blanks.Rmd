...
ps <- readRDS(paste0(dir_home,"/ps.rds"))
```

Alternatively, make the ps object
```{r makeps,eval=FALSE}
setwd("/Users/lauragivens/Downloads/Demo")
tax.tab <- read.csv("taxa.troph.csv",row.names = 1)
meta.tab <- read.csv("samplelist.csv",row.names = 1)

curated_lulu <- readRDS('curated_lulu.rds')
# Need to extract the table from the lulu object   
asv.tab <- curated_lulu$curated_table
# Column names will not match our sample names
# we need to change that  
cols_asv <- word(colnames(asv.tab),sep = "_",end=3) 
colnames(asv.tab) <- cols_asv

otu <- otu_table((asv.tab),taxa_are_rows = TRUE)
meta <- sample_data(meta.tab)
tax <- tax_table(as.matrix(tax.tab))

ps <- phyloseq(otu,meta,tax)
```

Or, load working environment:  
...
Check metadata  
```{r}
samplelist <- sample_data(ps) %>% as.matrix() %>% as.data.frame()
```
...
```{r}
ps.blanks <- ps %>% subset_samples(.,Rookery=='Blank') %>% prune_taxa(taxa_sums(.)>0,.)
...
We want to identify what taxa are in the blanks, and whether its better to remove all blank ASVs from the dataset or do some other method.  
...
```
Some of these are fisheries-relevant or otherwise target fish species.  
...
```{r}
sample_sums(ps %>% prune_taxa(taxa_names(.)%in%x,.)) %>% summary()
sample_sums(ps %>% prune_taxa(taxa_names(.)%in%y,.)) %>% summary()
```
...
```{r}
ps %>%  #sample sums when subset to only species in the blanks
  subset_taxa(.,Species%in%(get_taxa_unique(ps.blanks,"Species"))) %>% 
  sample_sums() %>% summary()

(ps %>% prune_taxa(!taxa_names(.)%in%taxa_names(ps.blanks),.)) %>%  #sample sums when subset to only species in the blanks
  subset_taxa(.,Species%in%(get_taxa_unique(ps.blanks,"Species"))) %>% 
  sample_sums() %>% summary()
```
...
```{r}
ps.decontam <- ps %>% subset_samples(Site != "Practice") %>% prune_samples(sample_sums(.)>0,.)
...
sample_data(ps.decontam)$is.neg <- sample_data(ps.decontam)$Site == "PCR"

sample_data(ps.decontam)$Quant_nguL[sample_data(ps.decontam)$Quant_nguL==0] <- 0.000000001

sample_data(ps.decontam)$Quant_nguL[is.na(sample_data(ps.decontam)$Quant_nguL)] <- 0.000000001
...
```{r}
sample_sums(ps %>% prune_taxa(taxa_names(.)%in%contamdf.names,.)) %>% summary()
```
...
```{r}
ps.blanks <- decontam.ps %>% subset_samples(.,Rookery=='Blank') %>% prune_taxa(taxa_sums(.)>0,.)
```
...
PCR blanks:  
```{r}
ps.blanks %>% subset_samples(Site=="PCR") %>% prune_taxa(taxa_sums(.)>0,.) %>% otu_table()
```
...
#in neg
otu_table(decontam.ps)[b[1],c]

#before
otu_table(decontam.ps)[b[1],a] #samples from that sampling event
...
for (p in 1:length(c)) { #for each negative control sample
  ...
  b <- tryCatch((prune_samples(sample_names(prune_a.ps)==c[p], #get only the blank sample we're working with
                     prune_a.ps) %>% 
    prune_taxa(taxa_sums(.)>0,.) %>% taxa_names()), #get seqid of asvs in neg control 
...
### Final
```{r}
prune.decontam.ps <- prune_c.ps %>% subset_samples(Rookery!="Blank")
```

# Overview   
Blanks  
```{r}
ps.blanks %>% plot_bar(x="SampleName_Short",fill="Genus") + facet_grid(~Site,scales="free") + labs(title="ASV abundance of negative controls and blanks",x="Sample Name") + theme(legend.position="none")
```
Before decontaminant removal  
```{r}
x <- ps %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
...
```{r}
y <- decontam.ps  %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
  plot_bar(x="SampleName_Short",fill="Genus") + facet_grid(~Site,scales="free") + 
...
```{r}
z <- prune.decontam.ps %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
  plot_bar(x="SampleName_Short",fill="Class") + facet_grid(~Site,scales="free") + 
...
```{r}
patchwork::wrap_plots(x,y,z) & theme(legend.position="none")
```


# Session Info   
```{r}
saveRDS(decontam.ps,paste0(dir_results,"/decontam.v2.ps.rds"))

saveRDS(prune_c.ps,paste0(dir_results,"/prune.decontam.v2.ps.keepblanks.rds"))
saveRDS(prune.decontam.ps,paste0(dir_results,"/prune.decontam.v2.ps.rds"))
```

```{r}
save.image(paste0(dir_data,'/07_Blanks_v2.RData'))
```
