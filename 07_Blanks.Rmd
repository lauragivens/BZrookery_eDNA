...
ps <- readRDS(paste0(dir_results,"/ps.troph.combined.wordsize.rds"))

taxa <- readRDS(paste0(dir_results,"/tax.troph.combined.wordsize.df.rds"))
...
#tax_table(ps) <- as.matrix(troph_edit)
...
head(check)
check$contaminant[!rownames(check)%in%(contamdf.names[contamdf.names%in%contamnames.prev])] <- FALSE  #only keep the one that was kept from prevanlence  
...
```{r}
ps %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
...
```{r}
decontam.ps  %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
  plot_bar(x="SampleName_Short",fill="Genus") + facet_grid(~Site,scales="free") + 
...
```{r}
prune.decontam.ps %>% subset_samples(Rookery!="Blank" & Rookery!="Practice") %>% prune_taxa(taxa_names(.)%in%taxa_names(ps.blanks),.) %>%
  plot_bar(x="SampleName_Short",fill="Genus") + facet_grid(~Site,scales="free") + 
...
# Session Info   
```{r}
saveRDS(decontam.ps,paste0(dir_results,"/decontam.combined.wordsize.ps.rds"))

saveRDS(prune_c.ps,paste0(dir_results,"/prune.decontam.combined.wordsize.ps.keepblanks.rds"))
saveRDS(prune.decontam.ps,paste0(dir_results,"/prune.decontam.combined.wordsize.ps.rds"))
```

```{r}
save.image(paste0(dir_data,'/07_Blanks_combined.wordsize.RData'))
```
